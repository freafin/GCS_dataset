...
import platform # new
from torch.utils.data import SequentialSampler # new
...
class TorchTrainMixedDataset:
    ...
    def get_loader(self, epoch) -> Iterable:
        ...
        for d_idx, (dataset, batch_size) in enumerate(
            ...
            #sampler = DistributedSampler(dataset, shuffle=self.shuffle) # original code
            #sampler.set_epoch(epoch) # original code

            # NEW Use DistributedSampler only if distributed training is initialized
            if dist.is_available() and dist.is_initialized():
                sampler = DistributedSampler(dataset, shuffle=self.shuffle)
                sampler.set_epoch(epoch)  # Only call set_epoch for DistributedSampler
            else:
                print("Skipping DistributedSampler on Windows or Single-GPU setup...")
                sampler = SequentialSampler(dataset)  # SequentialSampler works fine for single-GPU
