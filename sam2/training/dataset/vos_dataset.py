...
class VOSDataset(VisionDataset):
        ...
        for retry in range(MAX_RETRIES):
            try:
                ...
                if self.training:
                    ...
                    #raise e
                    # NEW:
                    logging.warning(f"Skipping val sample (id={idx}) due to: {e}")
    ...
    def __getitem__(self, idx):
        ...
        if datapoint is None:
            # Optionally log or count skipped samples
            raise IndexError(f"Validation sample {idx} is empty or invalid.")
        return datapoint
