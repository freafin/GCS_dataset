...
import platform # NEW
from torch.distributed import is_initialized # NEW
...
class Trainer:
    ...
    def __init__(
        ...
        *,  # the order of these args can change at any time, so they are keyword-only
        ...
        if platform.system() != "Windows":
            assert (is_initialized()), "Torch distributed needs to be initialized before calling the trainer."
        else:
            print("Skipping distributed initialization on Windows...")
    ...
    def _setup_torch_dist_and_backend(self, cuda_conf, distributed_conf) -> None:
        # If user disabled DDP in config, skip any torch.distributed calls: NEW
        if not distributed_conf.enabled:
            logging.info("Distributed disabled by config, skipping backend setup.")
            self.rank = self.distributed_rank = 0
            return

        #if not distributed_conf.enabled: # NEW 
        #    print("Distributed training is disabled via config.")
        #    self.rank=0
        #    return
        ...
        ## **Skip distributed setup if running on Windows** # NEW 
        #if platform.system() == "Windows":
        #    print("Skipping distributed setup on Windows...")
        #    self.rank = 0  # Define rank explicitly to prevent AttributeError
        #    return

        self.rank = setup_distributed_backend(
            distributed_conf.backend, distributed_conf.timeout_mins
        )
    # Ensure torch distributed is only checked when needed
    if platform.system() != "Windows" and is_initialized():
        is_dist_avail_and_initialized()
...
#
    ...
    def _setup_ddp_distributed_training(self, distributed_conf, accelerator):
        assert isinstance(self.model, torch.nn.Module)
        # Skip DDP wrapping if disabled in config *or* on Windows
        if not distributed_conf.enabled or platform.system() == "Windows":
            logging.info("Skipping DistributedDataParallel (enabled=%s, platform=%s)",
                         distributed_conf.enabled, platform.system())
            return
        ## **Skip DDP setup on Windows** # NEW 
        #if platform.system() == "Windows":
        #    print("Skipping DistributedDataParallel on Windows...")
        #    return
    
        # **Proceed with DDP setup only for multi-GPU cases**
        self.model = nn.parallel.DistributedDataParallel(
            self.model,
            device_ids=[self.local_rank] if accelerator == "cuda" else [],
            find_unused_parameters=distributed_conf.find_unused_parameters,
        ...
        if distributed_conf.comms_dtype is not None:  # noqa
            ...
            logging.info(f"Enabling {amp_type} grad communication")
            process_group = None
            self.model.register_comm_hook(process_group, hook)
    ...
    def _step(
        ...
        #    print(f"Debug - Model Outputs Keys: {outputs[0].keys()}")  # Verify available keys
        ...
        # Extract the model-selected mask
        if "pred_masks_high_res" in outputs[0]:  
            selected_mask = outputs[0]["pred_masks_high_res"]  # Directly retrieve the chosen mask
        else:
            logging.error("Error: `pred_masks_high_res` not found in outputs!")
            return {}, 0, {}

        # Compute binary accuracy - NEW
        selected_binary_preds = torch.sigmoid(selected_mask) > 0.5
        binary_targets = targets.float().reshape(selected_binary_preds.shape) # Ensure targets match the shape of predictions
        accuracy = torch.sum(selected_binary_preds == binary_targets).item() / binary_targets.numel()

        # Compute IoU - NEW
        intersection = torch.sum(selected_binary_preds * binary_targets).item()
        union = torch.sum(selected_binary_preds + binary_targets).item() - intersection
        iou = intersection / (union + 1e-6)

        ## **Compute Loss** NEW
        #print(f"Debug - Loss Keys: {self.loss.keys()}")
        ...
        # NEW (can be removed)
        if loss is None:
            logging.error(f"Loss calculation failed for key '{key}'. Returning default values.")
            return {}, 0, {}
        ...
        return StepResult(  # NEW  
            loss_dict={loss_str: loss},
            batch_size=batch_size,
            extra_losses=step_losses,
            accuracy=accuracy,
            iou=iou
        )
    ...
    def val_epoch(self, val_loader, phase):
        ...
        # Initialize validation accuracy and IoU tracking
        self.val_accuracy = 0
        self.val_iou = 0
        self.val_total = 0
        ...
        skipped_val_samples = 0 # NEW
        try:
            for data_iter, batch in enumerate(val_loader):
             
                if batch is None:
                    logging.warning(f"Skipping val sample {data_iter}: batch is None")
                ...
                try:
                    ...
                    with torch.no_grad():
                        with torch.cuda.amp.autocast(
                            ...
                            for phase, model in zip(curr_phases, curr_models):
                                ## Caputure accuracy and IoU from the _step method alongside loss
                                #loss_dict, batch_size, extra_losses, accuracy, iou = self._step(
                                #    batch,
                                #    model,
                                #    phase,
                                #)
                                ## Accumulate validation accuracy and IoU across batches - NEW
                                step_res = self._step(batch, model, phase)
                                loss_dict          = step_res.loss_dict
                                batch_size         = step_res.batch_size
                                extra_losses       = step_res.extra_losses
                                self.val_accuracy += step_res.accuracy * step_res.batch_size
                                self.val_iou      += step_res.iou * step_res.batch_size
                                self.val_total    += step_res.batch_size
                    ...
                    if dist.is_available() and dist.is_initialized(): # NEW
                        if data_iter % 10 == 0:
                            dist.barrier()
                    else:
                        print(f"Skipping validation barrier sync since DDP is not initialized") # NEW
                except Exception as e:
                    logging.warning(f"Validation error at sample {data_iter}: {e}")
                    skipped_val_samples += 1
                    continue

        except IndexError as e:
            logging.warning(f"Validation failed during loader iteration: {e}")
            skipped_val_samples += 1
            pass  # continue with rest of val_epoch or return early

        logging.info(f"Total skipped validation samples: {skipped_val_samples}")
        ...
        # Store all loss values for validation
        for k, v in loss_mts.items():
            out_dict[k] = v.avg
            #self.tensorboard_writer.add_scalar(f"Meters/{k}", v.avg, self.epoch) # NEW
        for k, v in extra_loss_mts.items():
            out_dict[k] = v.avg
            #self.tensorboard_writer.add_scalar(f"Meters/{k}", v.avg, self.epoch) # NEW

        # NEW - Compute final validation accuracy and IoU
        final_val_accuracy = self.val_accuracy / self.val_total if self.val_total > 0 else 0
        final_val_iou = self.val_iou / self.val_total if self.val_total > 0 else 0

        # NEW - Log accuracy and IoU in json and tensorboard
        out_dict["Metrics/Val_Accuracy"] = final_val_accuracy
        out_dict["Metrics/Val_IoU"] = final_val_iou
    ...
    def train_epoch(self, train_loader):
        ...
        # NEW - Tracking separate accuracy & IoU for loss-based and best IoU masks
        ...
        for data_iter, batch in enumerate(train_loader):
            ...
            try:
                #self._run_step(batch, phase, loss_mts, extra_loss_mts) # original code
                step_res = self._run_step(batch, phase, loss_mts, extra_loss_mts) # NEW 
                #loss_dict            = step_res.loss_dict
                #batch_size           = step_res.batch_size
                self.epoch_accuracy += step_res.accuracy * step_res.batch_size
                self.epoch_iou      += step_res.iou * step_res.batch_size
                self.epoch_total    += step_res.batch_size
        ...
        # NEW Final accuracy & IoU computation
        final_accuracy = self.epoch_accuracy / self.epoch_total if self.epoch_total > 0 else 0
        final_iou = self.epoch_iou / self.epoch_total if self.epoch_total > 0 else 0
        ...
        # NEW Add accuracy and IOU to logging output
        out_dict = self._log_meters_and_save_best_ckpts([Phase.TRAIN])
        out_dict["Metrics/Train_Accuracy"] = final_accuracy
        out_dict["Metrics/Train_IoU"] = final_iou
        ...
        self.logger.log_dict(out_dict, self.epoch) # NEW Logs metrics into Tensorboard
        return out_dict
    ...
    def _run_step(
        ...
        with torch.amp.autocast('cuda', #changed due to deprecation
            enabled=self.optim_conf.amp.enabled,
            dtype=get_amp_type(self.optim_conf.amp.amp_dtype),
        ):


            step_res: StepResult = self._step(batch, self.model, phase) # NEW 
            loss_dict    = step_res.loss_dict
            batch_size   = step_res.batch_size
            extra_losses = step_res.extra_losses
            accuracy     = step_res.accuracy
            iou          = step_res.iou 
        ...
        if not math.isfinite(loss.item()):
            ...
            if raise_on_error:
                ...
                #return None, batch_size, accuracy, iou # NEW
                return step_res
        ...
        #return loss_dict, batch_size, accuracy, iou # NEW
        return step_res
