...
from smp_dataset_transfer import SMPDatasetCellpose
...
model_name = f'Transfer_UNetplusplus_{epochs}Epochs_lr{lr}'
#%% Instantiate dataset
...
def train():
    transfer_model = torch.load(f"insert_path", weights_only=False).to(device) #map_location='cpu', 
    ...
    torch.save(transfer_model, f"insert_path")
...
data = pd.DataFrame(list(zip(epoch_losses, epoch_iou)), columns=['Loss', 'IOU'])
#%%
sns.lineplot(data=data)

#%%
test_model = torch.load(f"insert_path", 
...
test_data_vis = SMPDatasetCellpose(
    image_dir=test_img_dir,
    mask_dir=test_mask_dir,
    transform=test_transform,
    )
